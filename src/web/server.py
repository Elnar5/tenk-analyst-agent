...
Exposes:
  GET  /          → HTML interface
  POST /analyze   → upload PDF, get analysis (password-protected)
  POST /ask       → ask question about already-uploaded PDF (password-protected)
...
# ============================================================================
# Password protection — paid customers only
# ============================================================================
...
def is_valid_password(pw: Optional[str]) -> bool:
    ...
    if not pw:
        return False
    return pw.strip() in load_passwords()


def get_password_from_request(request: Request) -> Optional[str]:
    """Extract password from query param or header."""
    return (
        request.query_params.get("pw")
...
def home():
    """Editorial finance UI — Bloomberg/WSJ aesthetic with password gate."""
    return HTML_TEMPLATE


@app.post("/analyze")
async def analyze_pdf(request: Request, file: UploadFile = File(...)):
    """Upload a 10-K PDF and build the agent (password-protected)."""
    is_owner = request.query_params.get("source") == "owner"

    # Password check (skip for owner)
    if not is_owner:
    ...
    try:
        ...
        # Build agent (slow — 30–60s for embedding)
        ...
        return {"success": True, "chunks": chunk_count, "pages": page_count}
...
async def ask_question(request: Request, question: str = Form(...)):
    """Ask a question against the loaded agent (password-protected)."""
    is_owner = request.query_params.get("source") == "owner"
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .container { max-width: 880px; margin: 0 auto; padding: 60px 32px 80px; position: relative; z-index: 2; }
        .masthead {
            border-top: 3px double var(--ink); border-bottom: 1px solid var(--rule);
            padding: 24px 0 20px; margin-bottom: 40px;
            display: flex; justify-content: space-between; align-items: baseline;
        }
        .nameplate { font-size: 38px; font-weight: 700; line-height: 1; letter-spacing: -0.02em; font-style: italic; }
        .nameplate em { font-style: normal; color: var(--accent); }
        .tagline { font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: 0.18em; color: var(--ink-mute); margin-top: 8px; }
        .masthead-right { font-family: var(--mono); font-size: 11px; color: var(--ink-mute); text-align: right; text-transform: uppercase; letter-spacing: 0.1em; }
        .masthead-right .vol { font-weight: 700; color: var(--ink); }
        .lede { border-bottom: 1px solid var(--rule); padding-bottom: 32px; margin-bottom: 48px; }
        .deck { font-size: 22px; line-height: 1.5; color: var(--ink-soft); font-style: italic; max-width: 620px; }
        .deck strong { color: var(--ink); font-style: normal; }
        .byline { font-family: var(--mono); font-size: 10px; text-transform: uppercase; letter-spacing: 0.15em; color: var(--ink-mute); margin-top: 16px; }
        .section-num { font-family: var(--mono); font-size: 11px; font-weight: 700; color: var(--accent); letter-spacing: 0.18em; text-transform: uppercase; margin-bottom: 6px; }
        .section-head { font-family: var(--serif); font-size: 28px; font-weight: 600; line-height: 1.2; margin-bottom: 16px; letter-spacing: -0.01em; }
        .panel {
            background: rgba(255, 255, 255, 0.5); border: 1px solid var(--rule);
            padding: 28px 32px; margin-bottom: 32px; position: relative;
        }
        .panel::before { content: ''; position: absolute; top: -1px; left: -1px; right: -1px; height: 3px; background: var(--ink); }
        .panel.locked { opacity: 0.5; pointer-events: none; }
        input[type=file], input[type=text] {
            font-family: var(--mono); font-size: 14px; padding: 12px;
            color: var(--ink); display: block; margin: 12px 0 20px; width: 100%;
            background: var(--paper); border: 1px solid var(--rule);
        }
        input[type=file] { padding: 10px 0; border: none; }
        input[type=file]::file-selector-button {
            ...
            cursor: pointer; margin-right: 14px; transition: background 0.15s;
        }
        ...
        textarea {
            ...
            border-radius: 0; resize: vertical; transition: border-color 0.15s;
        }
        ...
        button:disabled { background: var(--ink-mute); cursor: not-allowed; }
        .status { font-family: var(--mono); font-size: 12px; margin-top: 14px; min-height: 18px; }
        .status.ok { color: var(--grounded); }
        .status.err { color: var(--accent); }
        .status.loading { color: var(--ink-mute); font-style: italic; }
        .answer-block {
            border-left: 3px solid var(--ink); padding: 20px 24px; margin-top: 24px;
        ...
        .answer-meta {
            ...
            color: var(--ink-mute); display: flex; justify-content: space-between; align-items: center;
            margin-bottom: 14px; padding-bottom: 10px; border-bottom: 1px dashed var(--rule);
        }
        .badge { display: inline-block; padding: 3px 10px; font-weight: 700; letter-spacing: 0.1em; border: 1px solid; }
        .badge.grounded { color: var(--grounded); border-color: var(--grounded); }
        .badge.not-grounded { color: var(--accent); border-color: var(--accent); }
        .answer-text { font-size: 18px; line-height: 1.65; color: var(--ink); white-space: pre-wrap; }
        .citations-list {
            margin-top: 18px; padding-top: 14px; border-top: 1px solid var(--rule);
            font-family: var(--mono); font-size: 11px; color: var(--ink-mute);
            text-transform: uppercase; letter-spacing: 0.08em;
        }
        .citations-list strong { color: var(--ink); margin-right: 8px; }
        .stats {
            display: grid; grid-template-columns: repeat(4, 1fr); gap: 0;
            border-top: 1px solid var(--rule); border-bottom: 1px solid var(--rule);
            margin: 48px 0; padding: 20px 0;
        }
        .stat { text-align: center; border-right: 1px dotted var(--rule); padding: 0 16px; }
        ...
        .footer {
            margin-top: 80px; padding-top: 32px; border-top: 3px double var(--ink);
            font-family: var(--mono); font-size: 10px; color: var(--ink-mute);
            text-transform: uppercase; letter-spacing: 0.15em;
            display: flex; justify-content: space-between; flex-wrap: wrap; gap: 16px;
        }
        .footer a { color: var(--ink-soft); text-decoration: none; border-bottom: 1px solid var(--rule); transition: color 0.15s; }
        ...
        .fade-in { animation: fade-up 0.4s ease-out; }
    </style>
</head>
<body>
    <div class="grain"></div>
    <div class="container">
        ...
        <div class="stats">
            <div class="stat"><div class="stat-num">92.3<span style="font-size:18px;">%</span></div><div class="stat-label">Pass Rate</div></div>
            <div class="stat"><div class="stat-num">100<span style="font-size:18px;">%</span></div><div class="stat-label">Citation Precision</div></div>
            <div class="stat"><div class="stat-num">100<span style="font-size:18px;">%</span></div><div class="stat-label">Hallucination Guard</div></div>
            <div class="stat"><div class="stat-num">0.69<span style="font-size:18px;">s</span></div><div class="stat-label">Median Latency</div></div>
        </div>
        ...
        <section class="panel locked" id="upload-panel">
            ...
            <button onclick="uploadPDF()" id="uploadBtn" disabled>Process Filing</button>
            <div class="status" id="uploadStatus"></div>
        </section>

        <section class="panel locked" id="ask-panel">
        ...
        <footer class="footer">
            <div>© 2026 · Independent</div>
            <div><a href="https://github.com/Elnar5/tenk-analyst-agent" target="_blank">Source on GitHub</a></div>
        </footer>
...
// Auto-fill access code if in URL or owner mode
window.addEventListener('DOMContentLoaded', () => {
    if (urlParams.get('source') === 'owner') {
        // Owner bypass — unlock everything
        accessCode = 'owner';
        unlockUI();
        document.getElementById('authStatus').className = 'status ok';
        document.getElementById('authStatus').textContent = '✓  Owner mode active.';
    } else if (urlParams.get('pw')) {
        document.getElementById('accessCode').value = urlParams.get('pw');
        verifyAccess();
    }
});

function unlockUI() {
    document.getElementById('upload-panel').classList.remove('locked');
    document.getElementById('ask-panel').classList.remove('locked');
...
function buildQueryString(extras) {
    ...
    if (accessCode && accessCode !== 'owner') params.set('pw', accessCode);
    for (const k in extras) params.set(k, extras[k]);
...
async function uploadPDF() {
    ...
    try {
        const res = await fetch('/analyze' + buildQueryString({}), { method: 'POST', body: formData });
        const data = await res.json();
        if (data.success) {
            status.className = 'status ok';
...
async function askQuestion() {
    ...
    try {
        const res = await fetch('/ask' + buildQueryString({}), {
            method: 'POST',
